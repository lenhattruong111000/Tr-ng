...
private boolean checkFullCandy() {
	...
	public ArrayList<Coordinate> checkSequenceCandy() {
		 ...
		 */
		int specialPos = 0; 
		boolean[][] visitRowSeq = new boolean[numberOfRow ][numberOfColumn ],
					visitColSeq = new boolean[numberOfRow ][numberOfColumn ];
	
		for (int i = numberOfRow - 1; i >= 0; i--)
			for (int j = numberOfColumn - 1; j >= 0; j--) {
				
		////////////
						if (!visitRowSeq[i][j]&& !visitColSeq[i][j] && hrow[i][j] == 3 ) {
							specialPos = -1; 
					
							if ( hrow[i][j]==3 && (hcol[i][j]==3||hcol[i][j-2]==3||hcol[i][j-1]==3||hcol[i+2][j-2]==3||
									hcol[i+2][j-1]==3||hcol[i+2][j]==3||hcol[i+1][j-2]==3||hcol[i+1][j]==3||hcol[i][j]==3) ) {
//								
									specialPos = i-1 ;
				...
				if (!visitRowSeq[i][j] && hrow[i][j] >= 3 ) {
					...
					if (hrow[i][j] == 4) {
						...
						specialPos = ((j - hrow[i][j] + 0) + j)/2 + 1; //supposed special position of the row
					...
					if (hrow[i][j] == 5) {
						...
						specialPos = ((j - hrow[i][j] + 0) + j)/2 + 1; //supposed special position of the row
						if (grid[i][specialPos] instanceof CandyNormal) {
							grid[i][specialPos] = new ChocolateCandyBomb();
						}
						else specialPos = -1;	
					}
...
//					append the coordinates (except which includes specialPos) to setList.
				...
				if (!visitColSeq[i][j] && hcol[i][j] >= 3 ) {
					...
					if (hcol[i][j] == 4) {
						...
						specialPos = ((i - hcol[i][j]) + i)/2 + 1; //supposed special position of the column
					...
					//append the coordinates (except which includes specialPos) to setList.
						...
						//Otherwise, set specialPos back to -1
						if (grid[specialPos][j] instanceof CandyNormal) {
							grid[specialPos][j] = new ChocolateCandyBomb(grid[specialPos][j].getColor());
						}
						else specialPos = -1;
				...
				if (!visitRowSeq[i][j] && hrow[i][j] ==1) {
						if (grid[i][j] instanceof ChocolateCandyBomb) {
							for (int p = 0; p < hrow[i][j]; p++) {
								visitRowSeq[i][j-p] = true; 
								if (j-p != specialPos) {
									coor = new Coordinate(i, j - p);
